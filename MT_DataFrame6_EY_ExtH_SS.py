...
from ind_var_SS.EY_ExtH_strand_specific import ey_eh_df
...
def make_dataframe(save_file=True, verbose=True):
    ...
    df_all_ey_eh_SS = pd.concat([df_all, ey_eh_df], axis=1, sort=False)

    # remove problematic text from outputs
    df_all_ey_eh_SS.replace('\r', ' ', regex=True, inplace=True)
    df_all_ey_eh_SS.replace('\n', ' ', regex=True, inplace=True)
    df_all_ey_eh_SS.replace(':', ' ',  regex=True, inplace=True)
    df_all_ey_eh_SS.replace(';', ' ',  regex=True, inplace=True)

    # replace NaN with NA
    df_all_ey_eh_SS = df_all_ey_eh_SS.replace('NaN', 'NA', regex=True)

    if verbose:

        # print dataframe
        print(df_all_ey_eh_SS)
        print("\n")

        # list column names and position
        for counter, i in enumerate(df_all_ey_eh_SS):
            print(counter, i)
        print("\n")

        # print dataframe info
        print("Columns:", df_all_ey_eh_SS.shape[1])
        print("Rows:", df_all_ey_eh_SS.shape[0])
        print("Datapoints:", df_all_ey_eh_SS.shape[0] * df_all_ey_eh_SS.shape[1])
        print("\n")
    ...
    if save_file:
        ...
        df_all_ey_eh_SS.to_csv(outfile, index=False, header=True)
...
# get file name for output
outfile_name = "EY_ExtH_20221129_filters_website.csv"
