...
from ind_var_SS.FB_strand_specific import feedback_ss_df
...
df_all_Feedback_SS = pd.concat([df_all, feedback_ss_df], axis=1, sort=False)

# remove problematic text from outputs
df_all_Feedback_SS.replace('\r', ' ', regex=True, inplace=True)
df_all_Feedback_SS.replace('\n', ' ', regex=True, inplace=True)
df_all_Feedback_SS.replace(':', ' ',  regex=True, inplace=True)
df_all_Feedback_SS.replace(';', ' ',  regex=True, inplace=True)

# replace NaN with NA
df_all_Feedback_SS = df_all_Feedback_SS.replace('NaN', 'NA', regex=True)

# list column names and position
for counter, i in enumerate(df_all_Feedback_SS):
    print(counter, i)

# useful info
print("Columns:", df_all_Feedback_SS.shape[1])
print("Rows:", df_all_Feedback_SS.shape[0])
print("Datapoints:", df_all_Feedback_SS.shape[0] * df_all_Feedback_SS.shape[1])
...
df_all_Feedback_SS.to_csv(outfile_name, index=False, header=True)

print(df_all_Feedback_SS)
