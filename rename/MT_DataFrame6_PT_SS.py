...
from ind_var_SS.PT_strand_specific import peer_tut_ss_df
...
df_all_PT_SS = pd.concat([df_all, peer_tut_ss_df], axis=1, sort=False)

# remove problematic text from outputs
df_all_PT_SS.replace('\r', ' ', regex=True, inplace=True)
df_all_PT_SS.replace('\n', ' ', regex=True, inplace=True)
df_all_PT_SS.replace(':', ' ',  regex=True, inplace=True)
df_all_PT_SS.replace(';', ' ',  regex=True, inplace=True)

# replace NaN with NA
df_all_PT_SS = df_all_PT_SS.replace('NaN', 'NA', regex=True)

# list column names and position
for counter, i in enumerate(df_all_PT_SS):
    print(counter, i)

# useful info
print("Columns:", df_all_PT_SS.shape[1])
print("Rows:", df_all_PT_SS.shape[0])
print("Datapoints:", df_all_PT_SS.shape[0] * df_all_PT_SS.shape[1])
...
df_all_PT_SS.to_csv(outfile_name, index=False, header=True)
