...
from ind_var_SS.CL_strand_specific import cl_ss_df
...
def make_dataframe(save_file=True, verbose=True):
    ...
    df_all_CL_SS = pd.concat([df_all, cl_ss_df], axis=1, sort=False)

    # remove problematic text from outputs
    df_all_CL_SS.replace('\r', ' ', regex=True, inplace=True)
    df_all_CL_SS.replace('\n', ' ', regex=True, inplace=True)
    df_all_CL_SS.replace(':', ' ',  regex=True, inplace=True)
    df_all_CL_SS.replace(';', ' ',  regex=True, inplace=True)

    # replace NaN with NA
    df_all_CL_SS = df_all_CL_SS.replace('NaN', 'NA', regex=True)
    ...
    if verbose:
        ...
        for counter, i in enumerate(df_all_CL_SS):
            print(counter, i)
        print("\n")

        # print dataframe info
        print("Columns:", df_all_CL_SS.shape[1])
        print("Rows:", df_all_CL_SS.shape[0])
        print("Datapoints:", df_all_CL_SS.shape[0] * df_all_CL_SS.shape[1])
        print("\n")
    ...
    if save_file:
        ...
        df_all_CL_SS.to_csv(outfile, index=False, header=True)
