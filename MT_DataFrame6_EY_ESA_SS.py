...
def make_dataframe(save_file=True, verbose=True):
    ...
    df_all_ey_esa_SS = pd.concat([df_all, ey_esa_df], axis=1, sort=False)

    # remove problematic text from outputs
    df_all_ey_esa_SS.replace('\r', ' ', regex=True, inplace=True)
    df_all_ey_esa_SS.replace('\n', ' ', regex=True, inplace=True)
    df_all_ey_esa_SS.replace(':', ' ',  regex=True, inplace=True)
    df_all_ey_esa_SS.replace(';', ' ',  regex=True, inplace=True)

    # replace NaN with NA
    df_all_ey_esa_SS = df_all_ey_esa_SS.replace('NaN', 'NA', regex=True)

    if verbose:

        # print dataframe
        print(df_all_ey_esa_SS)
        print("\n")

        # list column names and position
        for counter, i in enumerate(df_all_ey_esa_SS):
            print(counter, i)
        print("\n")

        # print dataframe info
        print("Columns:", df_all_ey_esa_SS.shape[1])
        print("Rows:", df_all_ey_esa_SS.shape[0])
        print("Datapoints:", df_all_ey_esa_SS.shape[0] * df_all_ey_esa_SS.shape[1])
        print("\n")
    ...
    if save_file:
        ...
        df_all_ey_esa_SS.to_csv(outfile, index=False, header=True)
...
# get file name for output
outfile_name = "EY_ESA_20221201_filters_website.csv"
