...
for section in range(len(data["References"])):
    if "Codes" in data["References"][section]:
        agefind, agelabel= exclude, exclude
        for study in range(len(data["References"][section]["Codes"])):
            for key, value in student_age.items():
                if key == data["References"][section]["Codes"][study]["AttributeId"]:
                    agefind, agelabel = value, key
        age.append(agefind)
    else:
        age.append(exclude)
...
df = pd.DataFrame(list(zip(itemids, titles, year, new, country, edusetting, pub, design, outcometext, interventiontext, studyrealism, partassignment, levelofassignment,
               ...
               columns =['ItemId', 'Author', 'Year', 'Strand', 'Country', 'EducationalSetting', 'PublicationType', 'StudyDesign', 'Outcome', 'Intervention', 'StudyRealism', 
                         'ParticipantAssignment', 'LevelofAssignment',  'StudentAge', 'StudentGender', 'SMD', 'SESMD', 'CIupperSMD', 
