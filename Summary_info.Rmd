...
for section in range(len(data["References"])):
    if "Codes" in data["References"][section]:
        levelassignfind, levelassignvalue = [], []
        perstudy=[]
        ...
        if len(perstudy) ==0:
            perstudy=exclude 
        levelofassignment.append(perstudy)
        if len(levelassignfind)==0:
            levelassignfind=exclude
        assigncomment.append([levelassignfind])
    else:
        levelofassignment.append(exclude)
        assigncomment.append(exclude)
```
...
for section in range(len(data["References"])):
    if "Codes" in data["References"][section]:
        ...
        if len(realismfind)==0:
            realismfind=exclude
        studyrealism.append(realismfind)
    else:
        studyrealism.append(exclude)
        studyrealismcomment.append(exclude)
```

```{python}
studentgender=[]
studentgendercomment=[]
for section in range(len(data["References"])):
    if "Codes" in data["References"][section]:
        genderfind, gendervalue = [], []
        perstudy=[]
        for study in range(len(data["References"][section]["Codes"])):
        ...
        if len(perstudy) ==0:
            perstudy=exclude       
        studentgendercomment.append(perstudy)
        if len(genderfind)==0:
            genderfind=exclude
        studentgender.append([genderfind])
    else:
        studentgender.append(exclude)
        studentgendercomment.append(exclude)
```
...
for section in range(len(data["References"])):
    if "Codes" in data["References"][section]:
        ...
        if len(perstudy)==0:
            perstudy=exclude
        agecomment.append(perstudy)
        if len(studentagefind)==0:
            studentagefind=exclude
        age.append(studentagefind)
    else:
        age.append(exclude)
        agecomment.append(exclude)
```
...
for section in range(len(data["References"])):
    if "Codes" in data["References"][section]:
        ...
        if len(perstudy) ==0:
            perstudy=exclude
        pubcomment.append(perstudy)
        if len(pubfind)==0:
            pubfind=exclude
        pub.append(pubfind)
    else:
        pub.append(exclude)
        pubcomment.append(exclude)
```
...
for section in range(len(data["References"])):
    if "Codes" in data["References"][section]:
        partassignfind, partassignvalue = [], []
        perstudy=[]
        for study in range(len(data["References"][section]["Codes"])):
        ...
        if len(perstudy)==0:
            perstudy=exclude
        partassigncomment.append(perstudy)
        if len(partassignfind)==0:
            partassignfind=exclude
        partassignment.append([partassignfind])
    else:
        partassignment.append(exclude)
        partassigncomment.append(exclude)
```
...
for section in range(len(data["References"])):
    if "Codes" in data["References"][section]:
        edusettingfind, edusettingvalue = [], []
        perstudy=[]
        for study in range(len(data["References"][section]["Codes"])):
        ...
        if len(perstudy)==0:
            perstudy=exclude
        edusettingcomment.append(perstudy)
        if len(edusettingfind)==0:
            edusettingfind=exclude
        edusetting.append([edusettingfind])
    else:
        edusetting.append(exclude)
        edusettingcomment.append(exclude)
```
...
for section in range(len(data["References"])):
    if "Codes" in data["References"][section]:
        ...
        if len(perstudy)==0:
            perstudy=exclude
        countrycomment.append(perstudy)
        if len(countryfind)==0:
            countryfind=exclude
        country.append(countryfind)
    else:
        country.append(exclude)
        countrycomment.append(exclude)
```
...
df = pd.DataFrame(list(zip(itemids, titles, year, strand_info, country, countrycomment, edusetting, edusettingcomment, pub, pubcomment, design, 
                           designcomment, outcometext, interventiontext, studyrealism, studyrealismcomment, partassignment, partassigncomment,
                           levelofassignment, assigncomment, age, agecomment, studentgender, studentgendercomment, SMD, SESMD, CIupperSMD,
...
df["SESMD"] = df["SESMD"].astype(float)
df["CIupperSMD"] = df["CIupperSMD"].astype(float)
df["CIlowerSMD"] = df["CIlowerSMD"].astype(float)

df["SMD"] = df["SMD"].round(4)
df["SESMD"] = df["SESMD"].round(4)
df["CIupperSMD"] = df["CIupperSMD"].round(4)
df["CIlowerSMD"] = df["CIlowerSMD"].round(4)
...
# convert all columns to character to save .csv (some were lists)
all <- all %>%
       mutate_all(as.character) %>%
       mutate_all(funs(str_replace_all(., "NaN", "NA")))

View(all)
#write.csv(all, "May12thExtraction_with_comments.csv", row.names=FALSE)
```
