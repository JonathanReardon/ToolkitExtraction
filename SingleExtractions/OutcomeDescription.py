        ...
        if "Outcomes" in data["References"][section]:
            for subsection in range(max(x)):
                if subsection < len(data["References"][section]["Outcomes"]):
                    outcomeIDholder.append(data["References"][section]["Outcomes"][subsection]["OutcomeDescription"])
                else:
...
outcome_description_df.columns=["Outcome_ID_"+'{}'.format(column+1) for column in outcome_description_df.columns]

outcome_description_df.fillna("NA", inplace=True)
