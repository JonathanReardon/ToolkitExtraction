...
```
**Function to check "Year" range, group by decade, and insert into new column**
...
```
**Data cleaning: replace all non-useful values with NA & convert columns to factors**
...
master_df$PublicationType <- as.character(master_df$PublicationType)
master_df$PublicationType[master_df$PublicationType=="NaN"] <- NA
master_df$PublicationType[master_df$PublicationType=="Other (Please specify)"] <- NA
master_df$PublicationType <- as.factor(master_df$PublicationType)
...
kable(Primary_feedback[1:10,1:13]) %>%
...
strand <- filter(master_df, !is.na(Strand)) %>%
...
intervention <- filter(master_df, !is.na(Intervention)) %>%
...
country <- filter(master_df, !is.na(Country)) %>%
...
edu_setting <- filter(master_df, !is.na(EducationalSetting)) %>%
...
pub_type <- filter(master_df, !is.na(PublicationType)) %>%
