...
def get_edu_info():
    ...
    for counter, value in enumerate(data["CodeSets"][2]["Attributes"]["AttributesList"][2]["Attributes"]["AttributesList"][2]["Attributes"]["AttributesList"]):
        ...
        #setting_name=mydata["CodeSets"][2]["Attributes"]["AttributesList"][2]["Attributes"]["AttributesList"][2]["Attributes"]["AttributesList"][counter]["AttributeName"]
        edu_setting.append(setting_code)
    return edu_setting[1]
    print(edu_setting)
...
def get_data(strand_key, strand_value, outcome_choice):
            ...
            if strand_id[0] == data["References"][section]["Codes"][study]["AttributeId"]:
                if "Outcomes" in data["References"][section]:
                      if data["References"][section]["Outcomes"][0]["OutcomeText"] == outcome_choice:
                          ...
                          controltext=(data["References"][section]["Outcomes"][0]["ControlText"])
                          outcome_studies.append([strand_key, strand_value, outcome_id, outcome_text, 
                                                  outcome_type, year, intervention, controltext, SMD, SESMD, CIupperSMD,
                                                  CIlowerSMD])
    ...
    df_primary = pd.DataFrame(outcome_studies, columns=['AttributeId', 'Strand', 'OutcomeId', 'OutcomeType', 
                                                        'ShortTitle', 'Year', 'Intervention', 'Control', 'SMD', 'SESMD',
                                                        'CIupper', 'CIlower'])
...
filter(master_df, !is.na(Intervention)) %>%
  ggplot(aes(Year, SESMD, color=Strand)) + geom_point(na.rm=TRUE) +
      theme_grey() +
