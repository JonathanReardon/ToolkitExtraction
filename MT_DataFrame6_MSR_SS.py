...
from ind_var_SS.MSR_strand_specific import msr_ss_df
...
df_all_msr_SS = pd.concat([df_all, msr_ss_df], axis=1, sort=False)

# remove problematic text from outputs
df_all_msr_SS.replace('\r', ' ', regex=True, inplace=True)
df_all_msr_SS.replace('\n', ' ', regex=True, inplace=True)
df_all_msr_SS.replace(':', ' ',  regex=True, inplace=True)
df_all_msr_SS.replace(';', ' ',  regex=True, inplace=True)

# replace NaN with NA
df_all_msr_SS = df_all_msr_SS.replace('NaN', 'NA', regex=True)

# list column names and position
for counter, i in enumerate(df_all_msr_SS):
    print(counter, i)

# useful info
print("Columns:", df_all_msr_SS.shape[1])
print("Rows:", df_all_msr_SS.shape[0])
print("Datapoints:", df_all_msr_SS.shape[0] * df_all_msr_SS.shape[1])
...
df_all_msr_SS.to_csv(outfile_name, index=False, header=True)
...
# get file name for output
outfile_name = "EY_SR_20221116_filters_website.csv"
